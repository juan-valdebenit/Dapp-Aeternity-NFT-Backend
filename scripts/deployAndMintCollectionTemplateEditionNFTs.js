...
(async function () {
  ...
  for (let i = 0; i < collectionTemplateEditionData.templates.length; i++) {
    const createTemplateTx = await contract.create_template(
      ...
      collectionTemplateEditionData.templates[i].edition_limit,
      { nonce }
    );
    ...
    for (let j = 0; j < i + 1; j++) {
      const templateMintTx = await contract.template_mint(
        senderAddress,
        i + 1,
        undefined,
        { nonce }
      ...
      console.log(
        `Minted NFT with id '${
          templateMintTx.decodedResult
        }' of template with id '${i + 1}'`
      );
