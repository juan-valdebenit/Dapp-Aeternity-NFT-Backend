...
const deployAndMintCollectionUniqueNFTs = async () => {
  ...
  const contractPath = "./contracts/CollectionUniqueNFTs.aes";
  const sourceCode = utils.getContractContent(contractPath);
  const fileSystem = utils.getFilesystem(contractPath);
  ...
  for (
    let i = 0;
    i < collectionUniqueMetadata.immutable_metadata_urls.length;
    i++
  ) {
    metadataMapAllNFTs.push(
      new Map([["url", collectionUniqueMetadata.immutable_metadata_urls[i]]])
    );
  }
  ...
  for (
    ...
    const mintTx = await contract.mint(
      senderAddress,
      { MetadataMap: [metadataMapAllNFTs[i]] },
      undefined,
      { nonce }
    );
